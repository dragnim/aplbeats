...
A rhythm is a rectangular array and a phrase is a vector. Array languages are good at both —
...
> generates a whole new rhythm or phrase from a recipe and a seed, **Apply with APL** transforms the
...
- **Two layers, one transport.** **Beats** is eight drum tracks; **Tones** is one pitched line.
...
- **A Tone phrase, sixteen steps.** One pitched voice over the same bar, played by one of six
...
- **Four recipes that create a whole Tone phrase in APL.** Pulse, Riff, Sparse and Climb, over a Root
...
- **Four more for the Tone phrase.** Transpose, Reverse, Rotate and Octave. Two of them are the same
...
- **Your session comes back** when you do — pattern, phrase, seed, preset, macros, locks, tempo,
...
It is that a phrase is a **different shape of data** from a rhythm, and the difference is the most
...
because you went to write a tune. Anything else would make the tabs a mode, and modes in a
...
pitched voice, so they were excluded for scope and never auditioned. The survey is reproducible with
...
loud the phrase was would be a volume control pretending to be an instrument control. That gain
...
### Transform a Tone phrase with APL

| Operation     | Expression           | What it does                         |
| ------------- | -------------------- | ------------------------------------ |
| **Transpose** | `(48⌈84⌊n+5)×0<n`    | Every note up or down, in semitones. |
| **Reverse**   | `⌽n`                 | The phrase backwards.                |
| **Rotate**    | `¯2⌽n`               | The phrase through time.             |
| **Octave**    | `(48⌈84⌊n+12×1)×0<n` | The whole phrase by whole octaves.   |

**Two of these are the same glyph the drums use.** `⌽n` reverses a phrase exactly as `⌽m` reverses
...
The other two exist because a phrase has something a rhythm has not: a value at each step. You
cannot transpose a Boolean.

There is **no Target control** here, and its absence is deliberate. A phrase is one line, so there
is nothing to choose between "this track" and "all tracks" — the control is missing rather than
present and disabled.

`48⌈84⌊` holds the result inside the instrument. A phrase transposed up past the top keeps its top
...
nothing else. Generating a phrase while a rhythm is in flight is refused by exactly the ref that
refuses two rhythms, which is the promise made to TryAPL.

The answers are a discriminated union on `domain`, so a phrase cannot be installed as a rhythm even
...
you transpose the phrase, you dislike it, you press Undo. Whichever thing you changed last is the
...
The phrase lives under `aplbeats.tones.v1` rather than in the session record, and that is not
tidiness. The session is discarded outright whenever the local generator's version changes, because
a stored _seed_ describes a different rhythm under a different generator. A phrase is sixteen
numbers that describe themselves. So after a generator bump the drums restart from the opening
groove and **the phrase is exactly where you left it**.
...
| Typing a seed, or New APL seed, or New Tone seed            | 0        |
...
| Editing a drum cell or a Tone note                          | 0        |
...
npm run verify:apl-tones-live            # two requests: every Tone recipe, every Tone transform
...
So it loads the published site, generates a rhythm and a Tone phrase, and checks that both came back
...
- **Offline APL.** There is no local fallback for any of the six APL tools, on purpose. No APL, no
  ...
  a stand-in for one.
- **Polyphony.** Tones is one voice. A chord is not expressible, and a second pitched line is not
  either.
...
| `npm run review:apl-tones`           | Judge the Tone recipes and transforms across many seeds, locally and for nothing         |
...
| `npm run verify:apl-tones-live`      | **Two real TryAPL requests** — every Tone recipe and transform. Manual, never in CI      |
...
src/
  apl/         everything that touches Dyalog APL, and nothing that does not
    ...
    toneOperations.ts  the four Tone operations
    toneGenerators.ts  the four Tone recipes, the five scales, the roots
    ...
    useToneApl.ts  the Tone phrase's state — and none of the machinery, which it is handed
    reference.ts   what the rhythm APL means, in TypeScript — imported by tests only
    toneReference.ts  the same for the Tone APL — imported by tests only
...
one request counter. `useToneApl` is handed `submit` and owns no machinery of its own, so a Tone
...
peaks at −1.9 dBFS; with a phrase over it that becomes −1.8 to −1.6, and **nothing clips even with
...
and some APL. About three hundred bytes for a rhythm and about a hundred for a phrase.
...
**Your session stays on your machine.** Pattern, phrase, seed, preset, macros, locks, tempo, swing,
...
```
now:     APL generates a rhythm or a phrase  →  APL transforms it, yours or ours  →  Web Audio plays
